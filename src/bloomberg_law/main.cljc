(ns bloomberg_law.main
  "Kotodama WASM entrypoint for the Bloomberg_law clean-room actor (L4) — Clojure port.
  ...
  facts (`bloomberg_law.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bloomberg_law")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"         :plural "matters"         :id-prefix "bloomber_mat"
    :fields [:clientId :name :status :openedAt]       :required [:name :status]
    :coerce {}                            :refs {}}
   {:entity "Document"       :plural "documents"       :id-prefix "bloomber_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"bloomberg_law.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bloomberg_law-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bloomberg_law-compat/ready)
